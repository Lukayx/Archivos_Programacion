...
-- funcion auxiliar para encontrar indice
findPos :: (Ord a) => a  -> [a] -> Int
findPos v [] = error "not found"
findPos v (x:xs) 
    | (v /= x) = 1 + (findPos v xs)
    | otherwise = 0
...
--      'medir [x1, x2, x3]'    -> obtiene una instancia del elemento (usted puede decidir cual)
--          
--    a) Crear el Typeclass 'Quantum'
class Quantum a where
    superPos :: a -> [a]
    medir :: [a] -> a
--    b) Hacer que Int y Char Instancien 'Quantum'
instance Quantum Int where
    superPos x    = [pred x, x, succ x]
    medir [x,y,z] = z

instance Quantum Char where
    superPos x = [pred x, x, succ x]
    medir [x,y,z] = x

--    c) Hacer que (Maybe m) instancie 'Quantum', explique bien el comportamiento que propone.
instance (Enum a) => Quantum (Maybe a) where
    superPos (Just x) = [Just (pred x), Just x, Just (succ x)]
